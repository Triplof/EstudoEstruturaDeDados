...
void bubbleSort(int *V  , int N){//otimizado
...
void mergeMat(struct aluno *V, int inicio, int meio, int fim){
    struct aluno *temp;
    ...
    p1 = inicio;
    p2 = meio+1;
    temp = (struct aluno *) malloc(tamanho*sizeof(struct aluno));
    if(temp != NULL){
        for(i=0; i<tamanho; i++){
            if(!fim1 && !fim2){
                if(V[p1].matricula < V[p2].matricula)
                    temp[i]=V[p1++];
                else
                    temp[i]=V[p2++];
...
void mergeSortMat(struct aluno *V, int inicio, int fim){
    int meio;
    if(inicio < fim){
        meio = floor((inicio+fim)/2);
        mergeSortMat(V,inicio,meio);
        mergeSortMat(V,meio+1,fim);
        mergeMat(V,inicio,meio,fim);
    }
}
...
//    int N = 10;
    ...
    //int N = 7;
    //int vet[] = {-8, 4, 23, 54, 67, 90, 21, 34, -3, 10};
    //int N = 10;

    int vet[] = {8,2,1,7,4,6,5};
    int N = 7;
